...
**The advertised schema admits the raw form; normalization runs in the schema.** Every normalization a `.describe()` promises (trim, case, zero-padding, prefix stripping, surrounding quotes) happens in the schema, and the handler never sees the raw form. A pattern input is built as `z.string().regex(RAW).transform(normalize).pipe(NORMALIZED)` (`normalizedString` in `input-schemas.ts`): `tools/list` (input-side JSON Schema) emits only the first stage, so `RAW` is written to admit every spelling the description promises (both cases, surrounding whitespace, blank), with no regex flags, since only the source is advertised. `NORMALIZED` then validates what the handler receives. A `z.preprocess` in front of a pattern would advertise the post-normalization pattern instead, and a client that validates arguments before sending would reject `hhs`, `7`, or `"363423"`. `RAW` runs on the unbounded caller string (up to the 1 MiB HTTP body limit), so it must fail in linear time: `rawPattern(core)` emits `^\s*(?:(?:core)\s*)?$`, the core starts and ends with non-whitespace, and no two adjacent quantified runs can match the same character. The opportunity-number pattern is written out as `^(?:[^"]*|\s*"[^"]*"\s*)$` for the same reason. `NORMALIZED` carries the length caps (`agencies` codes ≤ 100 characters, opportunity numbers ≤ 100), which also bound what an error echoes back. Enum inputs (`statuses`, `funding_instruments`, `sort`, `topic`) keep their canonical `enum`: their descriptions promise no other spelling, and case is still normalized server-side.
...
|:------|:-----|:-------------------------|:------|
| `keyword` | `optionalText(z.string().max(500))` | `keyword` (after [keyword compilation](#keyword-compilation)) | Full-text over title, description, number, agency. Bare terms are ANDed by the tool; OR mixed with AND (or with bare terms) must be parenthesized, and field prefixes (`agency:NSF`) are rejected. The compiled string is echoed as `effective_keyword`. |
...
| `agencies` | `optionalList(AGENCY_CODE_INPUT, 10)` (advertises a case-insensitive pattern) | `agencies`, pipe-joined, each code expanded and quoted as needed ([agency code encoding](#agency-code-encoding)) | `normalizeAgencyCode`: trim, uppercase (`hhs-nih11` → `HHS-NIH11`; lowercase returns 0 upstream), collapse internal whitespace runs to one space. `AGENCY_CODE = /^[A-Z0-9](?:[A-Z0-9 -]*[A-Z0-9])?$/`: ten real codes contain spaces (`DOT-FAA-FAA COE`, `DOT-FTA - TPM`, `DOT-DOT X-50`). Validated against the reference snapshot. Unknown → `unknown_agency`. |
| `eligibilities` | `optionalList(string of 1–2 digits \| integer 0–99 → zero-padded, 17)` | `eligibilities`, pipe-joined | Applicant-type codes, as strings or numbers. Single digits zero-padded (`7` → `07`; bare `7` returns 0 upstream). Validated against the snapshot; unknown → `unknown_eligibility`. OR within the list. |
...
| `funding_categories` | `optionalList(1–4 letters, any case → uppercased, 28)` | `fundingCategories`, pipe-joined | Uppercased (`hl` → `HL`; lowercase returns 0). Validated against the snapshot; unknown → `unknown_funding_category`. |
...
| `broadband OR internet rural` (= `broadband OR internet AND rural`) | No precedence applied to an ungrouped AND/OR mix: returns exactly `internet AND rural` (15); the OR branch is dropped. Grouped: `(broadband OR internet) AND rural` 17, `broadband OR (internet AND rural)` 29 | Rejected: `invalid_keyword`, with a hint spelling out both groupings. A grouping level (the whole keyword, or one pair of parentheses) may join its operands with AND or with OR, not both; bare adjacent terms count as AND. |
| `agency:NSF`, `cfda:93.866`, `title:broadband` | Field syntax → 0; stripping the colon instead ANDs the field name in as a term (`agency AND NSF` 46 vs `NSF` 119; `cfda AND "93.866"` 0) | A `word:value` prefix outside quotes is rejected: `invalid_keyword`, with a hint naming the filter that does the job (`agencies`, `assistance_listing`, `opportunity_number`, `eligibilities`, `funding_categories`, `funding_instruments`, `statuses`), or, for a text field (`title:`), dropping the prefix. A colon followed by a space or a symbol (`Healthy Start: …`, `https://…`) is not a prefix. |
| `broadband~`, `[a TO z]`, `^`, `\`, `{}`, `!`, `+` | `~` fuzzy; range → everything; `+` ignored | Characters `: ~ ? [ ] { } ^ \ / ! +` replaced by a space before tokenizing. `&&` → `AND`, `\|\|` → `OR`. |
...
`compileKeyword(raw): { ok: true, compiled } | { ok: false, problem, hint? }` is a pure function in the service layer. Steps: reject a field prefix → replace unsupported specials → tokenize (quoted phrases, parens, words) → classify operators → auto-quote internal-punctuation tokens → insert `AND` between adjacent operands → validate (balanced quotes/parens, no leading `AND`/`OR`, no trailing operator, no two binary operators adjacent) → reject a level mixing AND and OR → join. If nothing remains after stripping (keyword was only specials), the result is `invalid_keyword`. The handler maps `ok: false` to `ctx.fail('invalid_keyword', …)`, with `hint` as the recovery hint when present (field prefix, AND/OR mix).
...
| `keyword` compiled to ≥2 ANDed operands | `All keyword terms were required ({effective_keyword}); join alternatives with OR, in parentheses when other terms stay required, or drop a term.` |
...
| `assistance_listing` set | `assistance_listing is one exact ALN ({aln}); confirm the number (each grantsgov_get_opportunity record lists its assistance_listings), or drop assistance_listing and search the program name as keyword.` (`grantsgov_list_reference` has no ALN topic, so it is never the pointer here.) |
| any of `agencies`/`eligibilities`/`funding_categories`/`funding_instruments` set | `Remove one filter at a time, or confirm the {filters set} codes with grantsgov_list_reference.` |
...
|:-------|:-----|:-----|:---------|
| `invalid_keyword` | `ValidationError` | Keyword has unbalanced quotes or parentheses, a leading `AND`/`OR`, a trailing or doubled operator, nothing left after removing unsupported characters, a field prefix (`agency:NSF`), or AND mixed with OR in one grouping level. | `Fix the keyword syntax, or call grantsgov_list_reference with topic keyword_syntax for the supported operators.` (A field prefix gets a hint naming the matching filter; an AND/OR mix gets one spelling out both groupings.) |
...
|:------|:-----|:--------|:------|
| `opportunity_ids` | `optionalList(integer ≥ 1 \| digit string → number, 5)` | `fetchOpportunity` body `{ opportunityId }` | Accepts `363423` or `"363423"` (the search tool emits digit strings); the advertised item schema is `anyOf` integer or digit-string pattern. |
| `opportunity_numbers` | `optionalList(OPPORTUNITY_NUMBER_INPUT, 5)` | `search2` `oppNum` (double-quoted, all four statuses, `rows: 10`) → id → `fetchOpportunity` | Trimmed, and one pair of surrounding double quotes removed, as in search; internal spaces kept (`PAS-TUNIS- APS FY2026` is a real number). Quoted for the same reason as in search. Resolved as given; on no exact match with lowercase present, retried uppercased. Exact equality post-filter (case-insensitive), as in search. |
...
| Param | Type | Notes |
|:------|:-----|:------|
| `topic` | `z.preprocess(trimLower, z.enum(['agencies','eligibilities','funding_categories','funding_instruments','statuses','sort_options','keyword_syntax']))` | Required. Case and surrounding whitespace are normalized server-side; the advertised enum stays canonical. |
...
| `parent_code` | `optionalText(AGENCY_CODE_INPUT)` | `agencies` only: list every code under this one (`HHS` → its 58 sub-agencies; `DOD-DARPA` → its offices). Any other topic → `filter_not_applicable`. Unknown → `unknown_parent_code`. |
...
Agency counts are subtree counts: what an `agencies` filter on the code returns. A top-level agency's facet count already covers its sub-agencies (`HHS` 940 open), but a sub-agency's facet count covers only records filed at exactly that code, so the snapshot adds its descendants' counts (each record carries one agency code). `HHS-CDC` has 0 open records filed at its own code and 93 under its centers; it reports 93 open and 3,920 in all statuses, the same counts `grantsgov_search_opportunities` returns for `agencies: ["HHS-CDC"]`.
...
output: z.object({
  ...
  entries: z.array(z.object({
    ...
    open_count: z.number().optional().describe('Forecasted + posted opportunities with this code; for an agency, the code and every sub-agency under it.'),
...
| Stage | Rule |
|:------|:-----|
| Service text normalization (both surfaces) | Applied to `synopsisDesc`, `forecastDesc`, `applicantEligibilityDesc`, `fundingActivityCategoryDesc`, and `responseDateDesc`/`estApplicationResponseDateDesc`. These fields arrive either as HTML or as plain text with literal `\n` line structure and bare entities (DOS descriptions, NSF and DOD eligibility text), so the converter never treats input as whitespace-insensitive HTML: source newlines are kept, and tags only add breaks. Handled tags: `p`, `div`, `br` → line breaks; `li` → `- ` bullets (`ol` numbered); `strong`/`b`/`em`/`i`/`sup`/`span` → inner text; `a href` → `text (url)`; everything else stripped. HTML entities decoded (named table covering `&nbsp; &amp; &quot; &lt; &gt; &ldquo; &rdquo; &lsquo; &rsquo; &ndash; &mdash; &hellip;` plus numeric `&#…;`/`&#x…;`; unknown named entities left as-is). Titles (search and detail) are entity-decoded only (56 of 940 posted titles carry entities, e.g. `&rsquo;`). Runs of 3+ newlines collapse to one blank line; each line is right-trimmed. Every scan is linear in the input: tag attributes stop at the next `<`, comments and `script`/`style` elements are removed by a forward `indexOf`-style pass that stops at the first unterminated one, and list indentation stops growing past 6 levels, so hostile or garbled markup cannot stall the event loop or multiply the output. This normalized text is the canonical value in `structuredContent`, which otherwise stays verbatim: no line-break flattening and no markdown escaping. |
| `format()` inline slots | Any upstream string placed inside a line (title, agency name, opportunity id, category code, facet or reference label, file name, package title, related-opportunity note, close-date explanation in a table cell, contact name) has every line break flattened to one space: CRLF, LF, CR, and the Unicode breaks VT, FF, NEL (U+0085), LS (U+2028), and PS (U+2029), since a renderer or tokenizer may honor any of them. Inside a markdown table cell, `\|` is also escaped as `\\|`, and a backslash run directly before a pipe is doubled first, so a source `\|` cannot turn the added escape into an escaped backslash and split the row. |
| `format()` multi-line fields | `description`, `eligibility_narrative`, `funding_category_explanation`, `close_date_explanation` (when rendered as a block), `agency_contact.details`: split on the same line-break set, and each line is prefixed with `> ` (blank lines as `>`). This keeps agency text visibly quoted and keeps it from forging headings or tool-like structure in the rendered markdown. |
...
The text normalizer is about 100 lines in the service layer (`html-to-text.ts`) with no dependency. The markup is simple: a survey of 6 records found only `p ul ol li a strong span br sup div` and ten entity forms.
...
| `keyword`, AND/OR mix | `broadband OR internet AND rural` / `internet AND rural` / `(broadband OR internet) AND rural` / `broadband OR (internet AND rural)` / `broadband` (default statuses) | 15 / 15 / 17 / 29 / 19. An ungrouped mix drops the OR branch; each explicit grouping behaves as written. |
| agency counts | `agencies: "HHS-CDC\|HHS-CDC-*"` (the encoding of `HHS-CDC`), default and all statuses | 93 open / 3,920 total; the `HHS-CDC` sub-agency facet entry counts only records filed at exactly `HHS-CDC` (0 open, 1,728 total). |
...
| Tool handlers | Input normalization in the schemas (blank → unset, case, padding, digit-string ids, surrounding quotes), the advertised input-side JSON Schema admitting every raw form a description promises, `filter_conflict`/`unknown_*`/`invalid_keyword`/`no_identifiers` contracts, unconditional enrichment on every path (page, last page, zero hits, offset past end, closing-window empty), zero-hit notice composition, partial-success shape | Handler tests with `createMockContext({ errors: tool.errors })` over the fetch-mocked service; `getEnrichment(ctx)` for enrichment assertions; `expect.schemaMatching(tool.output)`. |
| `format()` | Parity (lint), line-break flattening in inline slots (CR/LF and the Unicode breaks), table-cell pipe escaping, blockquote rendering of multi-line fields, `Not available` for absent money/eligibility | Snapshot-free assertions on rendered text from sparse and full fixtures. |
| Robustness | Every advertised input pattern and `htmlToText` finish in linear time on 100 KB adversarial strings; the three tools survive `fuzzTool` valid and adversarial runs with no crash, leak, or prototype pollution | Timed unit tests; `tests/fuzz/tools.fuzz.test.ts` over the fetch-mocked service. |
...
| AND mixed with OR in one grouping level is rejected with both groupings spelled out, rather than grouped by a precedence rule. | Grants.gov applies no precedence to an ungrouped mix and silently drops the OR branch (`broadband OR internet rural` = `internet AND rural`). Callers split on which operator binds tighter (search engines read `a OR b c` as `(a OR b) c`; boolean convention as `a OR (b c)`), so either rule would silently run the other reading for some callers. A rejection costs one retry, and its hint gives both groupings to copy. |
| Field prefixes (`agency:NSF`, `cfda:93.866`) are rejected with a hint naming the matching filter, not mapped to that filter. | Stripping the colon ANDs the field name in as a term and silently narrows (`cfda:93.866` → 0). Mapping to a filter would change semantics without saying so (a keyword `NSF` is a full-text match, the `agencies` filter a subtree of codes) and has no faithful form inside `OR`/`NOT`/parentheses. |
| Pattern inputs advertise their raw forms and validate the normalized value in a later stage. | A `z.preprocess` in front of a pattern advertises the post-normalization shape, so a client that validates arguments against `tools/list` rejects spellings the description promises (`hhs`, `7`, `"363423"`) before the server sees them. |
| Raw patterns are written to fail in linear time, and length caps live in the normalized stage. | The raw stage runs on whatever string the caller sends, up to the HTTP body limit. Two adjacent runs that can claim the same whitespace backtrack polynomially: the direct spelling of the opportunity-number pattern, `^\s*(?:"[^"]*"\|[^"]*)?\s*$`, takes 92 s on 8,000 spaces followed by `"`, enough for one request to stall a shared HTTP deployment. The linear spellings accept the same strings (the ALN label's whitespace class also admits a tab), whereas a raw-stage `maxLength` would change what `tools/list` advertises. |
